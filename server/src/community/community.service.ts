...
import { COMMUNITY_DEFAULT_CATEGORIES, COMMUNITY_POST_LIMIT_DEFAULT, COMMUNITY_POST_LIMIT_MAX, COMMUNITY_VIEW_COUNT_WINDOW_MS, type CommunityCategoryKey } from './community.constants';
import {
...
const COMMUNITY_UPLOAD_DIR = join(process.cwd(), 'public', 'uploads', 'community');
...
const COMMUNITY_POST_MAX_IMAGE_COUNT = 6;
...
export class CommunityService {
  ...
  async createPost(currentUser: CurrentUserContext, dto: CreateCommunityPostDto) {
    ...
    const images = this.normalizeCreatePostImages(dto.images);
  ...
  async getPostDetail(
    ...
    const viewerHasFavorited = await this.loadFavoriteState(currentUser, [
      post.id,
    ]).then((set) => set.has(post.id));
  ...
  private normalizeCreatePostImages(
    ...
    if (images.length > COMMUNITY_POST_MAX_IMAGE_COUNT) {
      throw new BadRequestException('COMMUNITY_POST_IMAGE_LIMIT_EXCEEDED');
    }

    return images.map((image) => {
      const objectKey = trimCommunityText(image.objectKey);
      const url = trimCommunityText(image.url);
