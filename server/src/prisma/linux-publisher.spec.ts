...
import {
  ...
  getLinuxSourceStats,
  makeLinuxId,
  formatLinuxPublisherResult,
  parseLinuxPublisherMode,
  runLinuxPublisher,
  validateLinuxSource,
  type LinuxPublisherDatabase,
} from '../../scripts/publish-linux-fundamentals';
...
function createDatabase(options: { failAt?: number } = {}) {
  ...
  const findUnique = jest.fn(async ({ where }: any) => {
    if (where.slug !== LINUX_FUNDAMENTALS_SLUG) {
  ...
  const write = (delegate: string) =>
    jest.fn(async (args: any) => {
      ...
      return {
        id: args.create?.id ?? args.where?.id ?? 'course-linux',
      };
    });
  ...
  } as unknown as LinuxPublisherDatabase;
...
describe('Linux targeted publisher', () => {
  it('defaults to dry-run and rejects arbitrary course input', () => {
    expect(LINUX_FUNDAMENTALS_SLUG).toBe('linux-fundamentals');
    expect(parseLinuxPublisherMode([])).toBe('DRY_RUN');
    expect(parseLinuxPublisherMode(['--dry-run'])).toBe('DRY_RUN');
    expect(parseLinuxPublisherMode(['--apply'])).toBe('APPLY');
    expect(() => parseLinuxPublisherMode(['--course=python-basic'])).toThrow();
    expect(() => parseLinuxPublisherMode(['--dry-run', '--apply'])).toThrow();
  });

  it('locks source statistics and all deterministic identities', () => {
    const stats = getLinuxSourceStats();
    const plan = buildLinuxPublicationPlan();
    ...
    expect(plan[0]?.chapterId).toBe(
      makeLinuxId(
        'chapter',
        'linux-fundamentals:linux-fundamentals-chapter-01',
      ),
    );
  });
  ...
  it('dry-run reads only the fixed course baseline and performs no writes', async () => {
    ...
    const result = await runLinuxPublisher(fake.database, 'DRY_RUN');
    ...
    const report = formatLinuxPublisherResult(result);
    expect(report).toContain(
      'Baseline python-basic: course-python / Python basic / PUBLISHED / 1 chapters (1 published) / 1 lessons / 1 questions / 1 Battle',
    );
    expect(report).toContain(
      'Baseline data-structures-algorithms: NOT_FOUND / NOT_FOUND / NOT_FOUND / 0 chapters (0 published)',
    );
  });

  it('apply writes only course definition delegates in one transaction', async () => {
    const fake = createDatabase();
    const result = await runLinuxPublisher(fake.database, 'APPLY');
    ...
    expect(writes.filter((item) => item.delegate === 'quiz')).toHaveLength(10);
    expect(
    ...
    expect(
      writes
        ...
        .every((item) => item.args.where.slug === LINUX_FUNDAMENTALS_SLUG),
    ).toBe(true);
  });

  it('uses the same upsert identities on repeated apply', async () => {
    const fake = createDatabase();
    await runLinuxPublisher(fake.database, 'APPLY');
  ...
  it('rolls back every definition write on transaction failure', async () => {
    ...
    await expect(runLinuxPublisher(fake.database, 'APPLY')).rejects.toThrow(
  ...
  it('fails fast on source identity or count drift', () => {
    const wrongSlug = structuredClone(LINUX_FUNDAMENTALS_COURSE) as SeedCourse;
    wrongSlug.slug = 'python-basic';
    expect(() => validateLinuxSource(wrongSlug)).toThrow('source slug drifted');

    const wrongChapter = structuredClone(
      LINUX_FUNDAMENTALS_COURSE,
    ) as SeedCourse;
    wrongChapter.chapters[0]!.key = 'changed-chapter-key';
    expect(() => validateLinuxSource(wrongChapter)).toThrow(
    ...
    expect(() => validateLinuxSource(missingQuestion)).toThrow(
  ...
  it('contains no user, Python, delete, Battle history, or Rating write path', () => {
    const source = readFileSync(
      resolve(process.cwd(), 'scripts/publish-linux-fundamentals.ts'),
      'utf8',
    );
