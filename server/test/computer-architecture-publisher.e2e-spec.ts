import {
  ...
  parseComputerArchitectureOperatingSystemsPublisherMode,
  runComputerArchitectureOperatingSystemsPublisher,
} from '../scripts/publish-computer-architecture-operating-systems';

describe('Computer architecture targeted publisher', () => {
  it('defaults to dry-run and locks source statistics and stable identities', () => {
    expect(parseComputerArchitectureOperatingSystemsPublisherMode([])).toBe(
      'DRY_RUN',
    );
    ...
    expect(plan[0]?.chapterId).toBe(
      makeComputerArchitectureOperatingSystemsId(
        'chapter',
        'computer-architecture-operating-systems:computer-architecture-operating-systems-chapter-01',
      ),
    );
  });
