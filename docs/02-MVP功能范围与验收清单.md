...
* `GET /api/v1/users/me/learning` 用于学习中心展示“我的学习”课程列表。
* `GET /api/v1/users/me/learning` 只返回当前用户已产生学习记录的课程；无学习记录时返回 `success=true` 与空列表，不自动创建学习记录。
...
1. 每个章节最多绑定一个正式小测。
2. 当前阶段章节小测只支持单选题和判断题。
3. 获取小测时不得返回正确答案、判题结果或解析。
4. 未开始章节时允许查看小测，但不得自动创建学习记录。
5. 提交前必须已调用章节 `start`，且必须提交全部题目。
6. 最终得分、通过状态和逐题结果全部由服务端计算。
7. 每次合法提交都生成新的独立 `QuizAttempt`，历史记录不覆盖。
8. 小测结果页正确展示用户答案、标准答案和解析。
9. 小测提交成功仅代表具备完成章节资格，不自动完成章节。
10. 只要存在任意一次历史通过记录，就允许调用章节完成接口。
11. 未通过时保留历史记录，并允许无限次重新测试。
12. `QuizAnswer` 保留正式答题明细，作为后续错题本能力的数据来源。
