...
function mapBlocks(blocks: BattleContentBlock[], keyPrefix: string): ViewBlock[] {
...
function getReadableQuizError(error: unknown) {
  if (error instanceof RequestError) {
    ...
    if (error.code === 'QUIZ_NOT_READY' || error.code === 'QUIZ_NOT_PUBLISHED') {
...
function getReadableSubmitError(error: unknown) {
  if (error instanceof RequestError) {
    ...
    if (error.code === 'QUIZ_OPTION_INVALID' || error.code === 'QUIZ_QUESTION_INVALID') {
...
function normalizeQuestion(
  ...
  const correctOption = result
    ? sortedOptions.find((option) => option.optionId === result.correctOptionId) ?? null
    : null;
  ...
  return {
    ...
    stemBlocks: mapBlocks(question.stemBlocks, `${question.questionId}-stem`),
    ...
    options: sortedOptions.map((option, optionIndex) => {
      ...
      const isCorrect = result ? option.optionId === result.correctOptionId : false;
    ...
    result: result
      ? {
          isAnswered: Boolean(result.selectedOptionId),
          ...
          explanationBlocks: mapBlocks(
            result.explanationBlocks,
            `${question.questionId}-explanation`,
          ),
          hasExplanation: result.explanationBlocks.length > 0,
        }
      : null,
  };
}
  ...
  return [...questions]
    ...
    .map((question, index) =>
      normalizeQuestion(
        question,
        index,
        selectedAnswersMap[question.questionId] ?? '',
...
Page<ChapterQuizPageData>({
  ...
  async loadQuiz(chapterId?: string) {
    ...
    try {
      ...
      this.setData({
        ...
        title: quiz.title,
        description:
          quiz.description ?? '完成全部题目后提交，本页会直接显示每题正误、标准答案和解析。',
  ...
  async handleSubmit() {
    ...
    try {
      ...
      this.setData({
        state: 'submitted',
        questions: mapQuestions(latestQuizData.questions, selectedAnswers, result),
        scoreSummaryText: `本次得分 ${result.score}/${result.totalScore}，正确率 ${result.scorePercent}%`,
        feedbackHintText: '已展示本次作答的标准答案与解析，可返回章节继续学习。',
